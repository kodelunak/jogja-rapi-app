...
@section('title', __('Tambah Pembelian'))

@section('header')
    <h2 class="hidden sm:block text-xl font-semibold text-gray-800">{{ __('Tambah Pembelian') }}</h2>
@endsection
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-auto">
                    <h3 class="mb-4">{{ __('Tambah Pembelian') }}</h3>
                    @if ($errors->any())
                    ...
                    <form method="POST" action="{{ route('pembelian.store') }}" enctype="multipart/form-data">
                        ...
                        {{-- Jenis Supplier --}}
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Jenis Supplier</label>
                            <div class="flex gap-4 mb-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="supplier_type" value="existing" checked
                                        class="text-purple-600 focus:ring-purple-500">
                                    <span class="text-sm">Supplier Terdaftar</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="supplier_type" value="new"
                                        class="text-purple-600 focus:ring-purple-500">
                                    <span class="text-sm">Supplier Baru</span>
                                </label>
                            </div>

                            <div id="existing-supplier" class="mb-4">
                            ...
                            <div id="new-supplier" class="mb-4 hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="supplier_name" class="block text-sm font-medium text-gray-700">Nama Supplier Baru</label>
                                    <input type="text" name="supplier_name" id="supplier_name"
                                        placeholder="Masukkan nama supplier..."
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm">
                                </div>
                                <div>
                                    <label for="supplier_no_hp" class="block text-sm font-medium text-gray-700">No. HP</label>
                                    <input type="text" name="supplier_no_hp" id="supplier_no_hp"
                                ...
                                <div>
                                    <label for="supplier_alamat" class="block text-sm font-medium text-gray-700">Alamat</label>
                                    <input type="text" name="supplier_alamat" id="supplier_alamat"
                            ...
                            <div id="items-wrapper" class="space-y-3">
                                <div class="item-row grid grid-cols-1 sm:grid-cols-4 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="items[0][tanggal_expired]"
...
(function() {
    ...
    const radios = document.querySelectorAll('input[name="supplier_type"]');
    const existingDiv = document.getElementById('existing-supplier');
    const newDiv = document.getElementById('new-supplier');
    ...
    */
    ...
    wrapper.addEventListener('change', function(e) {
        if (e.target.matches('select[name^="items"][name$="[product_id]"]')) {
            ...
            // Try DOM attribute first (initial HTML options), fallback to PRODUCT_CATALOG
